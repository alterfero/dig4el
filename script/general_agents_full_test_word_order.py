...
#                 "sio", "cnt", "yur"]
experimental_candidates_lids = ["brm", "kho", "nav", "ngz", "nht", "nun", "otm", "tsh", "pal", "poh", "qim",
                "rot", "sdw", "san", "ngm", "spa", "sul", "mup", "svs", "swe",
                "twe", "tml", "ttn", "tgk", "tha", "len", "tsi", "tur", "tzu", "ung", "wal", "wol", "wlf",
                "sio", "cnt", "yur"]
...
if EXECUTE_ALGORITHM:
    # TOP ITERATION OF THE LEAVE ONE OUT PROCESS
    ...
    for excluded_lid in experimental_candidates_lids:
...
if ANALYZE_RESULTS:
    ...
    lang_acc = (
        run_acc.groupby("language_code", sort=False)["run_accuracy"]
    ...
    lang_order = (
        lang_acc.sort_values("median", ascending=False)["language_code"].tolist()
    )
    box_data_lang = [
        run_acc.loc[run_acc.language_code == code, "run_accuracy"].tolist()
    ...
    make_boxplot(
        ...
        ylabel="Accuracy (0–1)",
        filename=Path("accuracy_per_language_boxplot.png"),
    )
    ...
    make_boxplot(
        ...
        filename=Path("accuracy_per_parameter_boxplot.png"),
        rotation=45,
    )
    ...
    make_boxplot(
        ...
        filename=Path("language_median_accuracy_boxplot.png"),
        rotation=0,
        figsize=(4, 6),
    )
