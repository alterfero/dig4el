...
with colp.popover("i"):
    st.markdown(
        "This page allows to test General Agents. Choose one or multiple general domains, then select languages, either by hand-picking them, "
    ...
    st.markdown(
        "This is the prototype of a work in progress, features will evolve and behaviors will improve over time.")
...
fl = st.multiselect("Choose languages instead of random selection", list(wu.language_pk_id_by_name.keys()),
                    placeholder="Leave empty for a random language selection")
...
if len(st.session_state["ga_domains"]) > 0:
    ...
    if not st.session_state["languages chosen by user"]:
        ...
        if st.session_state["language_family_filter"] != "ALL":
            ...
            st.write("Focusing on the {} languages from the {} family.".format(len(family_filtered_language_id_list),
                                                                               st.session_state[
                                                                                   "language_family_filter"]))
        else:
        ...
        if len(available_languages) > 0:
            ...
            if st.checkbox("Show language list"):
            ...
            if st.session_state["new_user_params"] or st.session_state["languages_used_for_testing"] == []:
                if st.session_state["number_of_languages_used_for_testing"] < len(available_languages):
                    st.session_state["languages_used_for_testing"] = random.sample(available_languages,
                                                                                   st.session_state[
                                                                                       "number_of_languages_used_for_testing"])
                else:
    ...
    if len(st.session_state["languages_used_for_testing"]) > 0 and (
            st.session_state["observed_params_name"] != [] or st.session_state["new_user_params"]):
    ...
    if len(st.session_state["languages_used_for_testing"]) > 0:
        ...
        if st.session_state["observed_params_name"] != [] or st.session_state["new_user_params"]:
            ...
            for lid in st.session_state["languages_used_for_testing"]:
                ...
                st.session_state["expected_beliefs"] = {}
                infospot.write(
                    "establishing priors, injecting beliefs and running messaging until consensus on language **{}**...".format(
                        wu.language_info_by_id[lid]["name"]))
                ...
                if not st.session_state["peak_beliefs_injected"]:
                    for lp_name in st.session_state["current_ga"].language_parameters.keys():
                        if lp_name in st.session_state["observed_params_name"]:
                            ...
                            st.session_state["current_ga"].language_parameters[lp_name].inject_peak_belief(true_depk,
                                                                                                           st.session_state[
                                                                                                               "prob_obs"],
                                                                                                           locked=True)
                ...
                for upn in st.session_state["unknown_params_names"]:
                    ...
                    expected_value = wu.get_wals_language_data_by_id_or_name(lid)[int(wu.parameter_pk_by_name[upn])][
                        "domainelement_pk"]
                    current_consensus = max(st.session_state["current_ga"].language_parameters[upn].beliefs,
            ...
            if st.session_state["current_ga"] is not None:
                ...
                st.metric("**Average accuracy guessing unknown values through consensus**",
