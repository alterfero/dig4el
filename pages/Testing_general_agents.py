...
if len(st.session_state["ga_domains"]) > 0:
  ...
  if not st.session_state["languages chosen by user"]:
    ...
    if len(available_languages) > 0:
      ...
      st.warning("Not enough available parameters of these domains in the languages of this family. Reduce the minimum number of parameters or change/add General Agent domains")
  ...
  if len(st.session_state["languages_used_for_testing"]) > 0 and (st.session_state["observed_params_name"] != [] or st.session_state["new_user_params"]):
    ...
    if st.session_state["observed_params_name"] == [] or st.session_state["new_user_params"]:
      ...
      # filter with available_param_pk_by_name representing our grammatical parameters of interest.
      available_params_pk_in_target_language = []
      for ppk in general_available_params_pk_in_target_language:
        if ppk in st.session_state["available_param_pk_by_name"].values():
      ...
      for ppk in available_params_pk_in_target_language:
        if ppk in st.session_state["observable_params_pk_by_name"].values():
